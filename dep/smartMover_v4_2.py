...
class Player:
    ...
    def move(self, board, time):
        try:
            print(hi)
            return chess.polyglot.MemoryMappedReader("data/corr.bin").weighted_choice(board).move
        except:
            start = t.time()
            self.ply = 0
            #If checks:#
            ...
            while t.time() - start <= 2:
                ...
                for move in moves:
                    board.push(move)
                    moveScore = self.getValue(board, 1, 1, alpha, beta, time)
                    board.pop()
                ...
                print("Color:",self.color,"Nodes:",self.ply,"Move:",bestMove,"Score:",bestMoveScore,"Ply:",self.ply,"Time:",t.time() - start)
            return bestMove
    ...
    def getValue(self, board, currentDepth, agentIndex, alpha, beta, time):
        self.nodes += 1
        if currentDepth >= self.ply or board.is_game_over():   
    ...
    def maxValue(self, board, currentDepth, alpha, beta, time):
        ...
        for move in list(board.legal_moves):
            board.push(move)
            maxValue = max(maxValue, self.getValue(board, currentDepth + 1, 1, alpha, beta, time))
            board.pop()
